...
public class pnNameThemThanhVienDuAn extends JPanel {
	...
	protected pnNameThemThanhVienDuAn(final NguoiDung mem2) {
		this.mem = mem2;
		setSize(280, 30);
		...
		for (NguoiDung nd : CongViecPanel.memberList) {
		...
		state.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (isCheck == 0) {
					state.setIcon(check.getIcon());
					isCheck = 1;
					Search.sendAddMemberToProject(R.congViecPanel.getDuAn().getMaDuAn(), mem.getUserID());
				} else {
					state.setIcon(uncheck.getIcon());
					isCheck = 0;
					Search.sendDelMemberFromProject(R.congViecPanel.getDuAn().getMaDuAn(), mem.getUserID());
				}
			}
		});
